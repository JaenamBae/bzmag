================================================================================
    MFC 라이브러리 : bzMag 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 bzMag 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

bzMag 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
bzMag.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 bzMagApp 응용 프로그램 클래스가 선언되어 있습니다.

bzMag.cpp
    이는 응용 프로그램 클래스 bzMagApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\bzMag.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 bzMag.rc에 의해 포함됩니다.
...
MainFrm.h, MainFrm.cpp
    이 파일에는 프레임 클래스인 MainFrame가 포함되어 있습니다.
    이는 CFrameWnd에서 파생되었으며 모든 SDI 프레임 기능을 제어합니다.

res\Toolbar.bmp
    이 비트맵 파일은 도구 모음의 바둑판식 이미지를 만드는 데 사용됩니다.
    초기 도구 모음과 상태 표시줄은 MainFrame 클래스에서 생성됩니다. 이 도구 모음 비트맵을 리소스 편집기로 편집하고, bzMag.rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음 단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////

응용 프로그램 마법사에서 문서 형식과 뷰를 한 개씩 만듭니다.

bzMagDoc.h, bzMagDoc.cpp - 문서
    이 파일에는 bzMagDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 bzMagDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      bzmag
        파일 형식 ID:        bzMag.Document
        주 프레임 캡션:  bzMag
        Doc 형식 이름:       bzMag
        필터 이름:         bzMag Files (*.bzmag)
        파일의 새 약식 이름: bzMag
        파일 형식의 긴 이름: bzMag.Document

bzMagView.h, bzMagView.cpp - 문서 뷰
    이 파일에는 bzMagView 클래스가 포함됩니다.
    bzMagView 개체는 bzMagDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 bzMag.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.
