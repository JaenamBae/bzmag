...
BEGIN_MESSAGE_MAP(MultiSelectTreeCtrl, CTreeCtrl)
	...
	ON_NOTIFY_REFLECT(TVN_ITEMCHANGING, &MultiSelectTreeCtrl::OnTvnItemChanging)
	ON_NOTIFY_REFLECT(TVN_SELCHANGED, &MultiSelectTreeCtrl::OnTvnSelchanged)
END_MESSAGE_MAP()
...
void MultiSelectTreeCtrl::OnTvnItemChanged(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	MultiSelectTreeView* pView = dynamic_cast<MultiSelectTreeView*>(GetParent());
	if (pView) pView->OnTreeItemChanged(pNMHDR, pResult);

	*pResult = 0;
}


void MultiSelectTreeCtrl::OnTvnItemChanging(NMHDR *pNMHDR, LRESULT *pResult)
{
	NMTVITEMCHANGE *pNMTVItemChange = reinterpret_cast<NMTVITEMCHANGE *>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;
...
void MultiSelectTreeCtrl::OnTvnSelchanged(NMHDR *pNMHDR, LRESULT *pResult)
{
